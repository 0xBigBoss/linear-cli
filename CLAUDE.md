...
- Command surface: global `--json`/`--config`/`--help`/`--version`; subcommands `auth set|test`, `me`, `teams list`, `issues list`, `issue view`, `issue create`, `gql`. Pagination is a stub—warn when more pages remain.
